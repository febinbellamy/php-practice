...
function trilingualDemocracy($group) {
    ...
    for($i = 0; $i < strlen($group);$i++) {
      ...
      if (array_key_exists($currentChar, $languagesCount)) {
        $languagesCount[$currentChar]++;
      } else {
        $languagesCount[$currentChar] = 1;
      }
    }
    ...
    foreach ($languagesCount as $language => $count) {
      if ($count === 3) {
        ...
        if ($keyWithOneCount) return $keyWithOneCount;
        $returnKey = true;
      } elseif ($returnKey) {
        return $language;
      } elseif ($count === 1) {
        $keyWithOneCount = $language;
      }
    }
  
    $languages = "FIKD";
    for($j = 0; $j < strlen($languages); $j++) {
      $language = $languages[$j];
      if (!array_key_exists($language, $languagesCount)) {
        return $language;
      }
    }
  }