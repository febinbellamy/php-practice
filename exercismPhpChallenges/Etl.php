...
/* 
https://exercism.org/tracks/php/exercises/etl/
...
{
    $result = [];
    foreach ($input as $currentPoints => $currentLettersArr) {
        for ($i = 0; $i < count($currentLettersArr); $i++) {
            $currentLetter = strtolower($currentLettersArr[$i]);
            $result[$currentLetter] = (int)$currentPoints;
        }
    }
    return $result;
}
