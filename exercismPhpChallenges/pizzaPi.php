...
class PizzaPi
{
    public function calculateDoughRequirement($numOfPizzas, $numOfPersonsPerPizza)
    {
        return $numOfPizzas * (($numOfPersonsPerPizza * 20) + 200);
    }
    ...
    public function calculateCheeseCubeCoverage($sideLengthDimension, $desiredThickness, $diameterOfPizza)
    {

        return floor( ($sideLengthDimension ** 3) / ($desiredThickness * pi() * $diameterOfPizza) );
    }

    public function calculateLeftOverSlices($numOfPizzas, $numOfFriends)
    {
        return ($numOfPizzas * 8) % $numOfFriends;
    }
}
